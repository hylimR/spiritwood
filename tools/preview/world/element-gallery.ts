 ...
 * Usage: node tools/preview/world/element-gallery.ts [outDir] [--zoom 1] [category or spec key…]
 */
...
const specs = ELEMENT_SPECS.filter((s) => args.length === 0 || args.includes(s.category) || args.includes(s.key ?? ''));
    ...
    for (const el of line) {
      for (let y = 0; y < el.h * zoom; y++) {
        for (let xx = 0; xx < el.w * zoom; xx++) {
          ...
          shadeKit(out, tex, 0.5, glow, 0, st.p, KIT_MODE.Band);
...
save(dir, `gallery-${args.join('-') || 'all'}.png`, rgba, W, H);

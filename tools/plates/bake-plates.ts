...
async function main(): Promise<void> {
  ...
  console.log(`painting ${plan.width}×${plan.height} texels (texelScale ${TEXEL_SCALE}) for ${REPLACES} (f ${fx})`);
  const t0 = performance.now();
  ...
  const chunks = await bakeImage(img, plan.layer.id);
  const out = plateManifest(manifest, plan, chunks);
  parseManifest(out);
  ...
  console.log(`wrote ${outPath}; plate assets ${(total / 1024 / 1024).toFixed(2)} MB; baked in ${((performance.now() - tStart) / 1000).toFixed(1)} s`);

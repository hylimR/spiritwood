# Split hull listings (channel-packed-atlas)
...
2. Column cells, `cell` texels wide. Each cell gets visible spans (snapped outward, small gaps merged, capped at `maxSpans`) and, per span,
   the longest fully-core run (snapped inward, kept if at least `minCore`).
3. A sweep across the columns merges rects with an identical `(y0, y1, kind)` into wide rects.
4. `computeSplitHullHalf` runs the split on a conservative 2× downsample (visible if any texel is, opaque only if all are). It is 4× cheaper and keeps the invariants.
...
`src/render/gen/hull.ts` lines 1–283:
...
## Invariant test (keep this whenever you change the options)

This is from `tests/world/hull.test.ts`. The file also covers: an empty map, a fully opaque map, a soft disc, random blobs with holes,
the `maxSpans` cap, horizontal merging, determinism, and row splitting. These tests exercise the full-resolution `computeSplitHull`.
For the half-resolution path the kit ships, `tests/world/kit.test.ts` checks that every core rect of the generated forest kit lies on
α ≥ 254 texels and that `ELEMENT_MARGIN ≥ KIT_HULL.pad`.

`tests/world/hull.test.ts` lines 5–68:

...
import { coverage, opIntersect, sdCircle, sdEllipse, sdRoundBox, sdTaperedCapsule, smin } from '../gen/sdf.ts';
 ...
 * Gloomcrawler and the Moonwell shrine, plus shared glow/beam/sparkle/wisp images. Stone is shaded as a
 * dark mass with a cool moonlight rim from the upper left, so it reads against the night forest.
 */
...
type Shade = (x: number, y: number, d: number, nx: number, ny: number, out: number[]) => number;
...
function bake(spec: ImageSpec): AtlasImage {
...
function radial(name: string, size: number, fn: (r: number) => number, color: readonly number[] = [1, 1, 1]): AtlasImage {
...
const noise = new Noise(0x57a1);

/** Dark stone with a pillow normal, moonlight rim, grain and cracks. */
function stoneShade(thickness: number, moss: (x: number, y: number) => number): Shade {
...
export function buildEntityAtlas(): Atlas {
  return packAtlas(buildEntityImages(), ENTITY_ATLAS_WIDTH, ENTITY_GUTTER);
}
